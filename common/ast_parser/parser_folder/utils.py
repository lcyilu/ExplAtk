...
def tree_to_function_index(root_node, index_to_code):
    """
    获取所有函数名的索引位置
    """
    function_indices = []
    
    def traverse_node(node, parent_type=None):
        if node is None:
            return
        
        # C/C++函数声明和定义的模式
        if node.type == 'function_declarator':
            # 找到函数名（通常是第一个identifier子节点）
            for child in node.children:
                if child.type == 'identifier':
                    function_indices.append((child.start_point, child.end_point))
                    break
            ...
            for child in node.children:
                if child.type == 'function_declarator':
                    traverse_node(child, 'function_definition')
                else:
                    traverse_node(child, node.type)
