...
from common.ast_parser.parser_folder import (
    index_to_code_token,
    remove_comments_and_docstrings,
    tree_to_token_index,
)
from common.ast_parser.parser_folder.DFG_c import DFG_c
from common.ast_parser.parser_folder.DFG_java import DFG_java
from common.ast_parser.parser_folder.DFG_python import DFG_python
...
def _javalang_tokenize(line: str):
    import javalang

    return javalang.tokenizer.tokenize(line)
...
def _default_language_so_path() -> str:
    return get_settings().language_so_path


_LANG_SO_PATH_LOADED: str | None = None
_parsers: dict[str, Any] | None = None
_EXTRA_TS_PARSERS: dict[tuple[str, str], Parser] = {}
...
def _ensure_parsers(language_so_path: str | None = None) -> None:
    global _parsers, _LANG_SO_PATH_LOADED, _EXTRA_TS_PARSERS
    path = language_so_path or _default_language_so_path()
    if _parsers is not None and path == _LANG_SO_PATH_LOADED:
        return
    _EXTRA_TS_PARSERS.clear()
    if not os.path.exists(path):
        for _lang in LANG_LIB_MAP:
            repo = LANG_REPO_MAP.get(_lang)
            if repo and not os.path.exists(repo):
                os.popen(
                    f"git clone https://github.com/tree-sitter/{repo}.git"
                ).read()
    ...
    for lang in dfg_function:
        ...
        _parsers[lang] = [parser, dfg_function[lang]]
    _LANG_SO_PATH_LOADED = path
...
def extract_dataflow(code, lang, language_so_path=None):
    _ensure_parsers(language_so_path)
...
def get_example(code, tgt_word, substitute, lang, language_so_path=None):
    _ensure_parsers(language_so_path)
    parser = _parsers[lang]
    code = code.replace("\\n", "\n")
    parser = _parsers[lang]
...
def get_example_batch(code, chromesome, lang, language_so_path=None):
    _ensure_parsers(language_so_path)
    parser = _parsers[lang]
    code = code.replace("\\n", "\n")
    parser = _parsers[lang]
...
def get_identifiers_ori(code, lang, language_so_path=None):
    dfg, index_table, code_tokens = extract_dataflow(code, lang, language_so_path)
    ret = set()
    for d in dfg:
...
def get_identifiers(code, lang, only_code_tokens=False, language_so_path=None):
    _ensure_parsers(language_so_path)
    code = code.replace("\\n", "\n")
    try:
        code = remove_comments_and_docstrings(code, lang)
    ...
    ret = []
    if lang == 'python':
        dfg, index_table, _ = extract_dataflow(code, lang, language_so_path)
        variable_name = []
        function_name = []
        for d in dfg:
            if is_valid_variable_name(d[0], lang):
        ...
        dfg, _, _ = extract_dataflow(code, lang, language_so_path)
        variables = set()
        for d in dfg:
            if is_valid_variable_name(d[0], lang):
                variables.add(d[0])
    ...
    if random.random() <= code_style[0][0]:
        if lang == 'c':
            ...
            while source_code.find('printf("', end) != -1:
                ...
                replacement = 'char ' + all_variable_name[0] + '['+ str(len(string)) +'] = ' + string + ';' + \
                              'printf("%s",' + all_variable_name[0] + ')'
                source_code = pre_code + replacement + after_code
                # 跳过刚插入的部分，避免再次匹配到新的 printf("
                end = begin + len(replacement)
        ...
        if lang in ['c', 'java', 'python']:
            ...
            for combined_assignments in all_combined_assignments:
                ...
                while source_code.find(combined_assignments, index) != -1:
                    ...
                    # 跳过刚替换的部分，防止重复匹配
                    index += len(temp_var) + 1 + len(temp_var) + len(combined_assignments) - 1
        all_source_codes.append(source_code)
                ...
                for flag in ['true', 'false']:
                    ...
                    while temp_line.find(flag, begin) != -1:
                        ...
                        if len(all_variable_name) > 1:
                            ...
                            all_variable_name[0] += '_'
                tokens = _javalang_tokenize(temp_line)
                try:
...
def _normalize_ts_lang(lang: str) -> str:
...
def parse_code_to_ast(code: str, lang: str, *, language_so_path: str | None = None):
    """Parse source with tree-sitter and return the root ``tree_sitter.Node``."""
    global _EXTRA_TS_PARSERS
    lang = _normalize_ts_lang(lang)
    _ensure_parsers(language_so_path)
    if lang in _parsers:
        p = _parsers[lang][0]
    else:
        path = language_so_path or _LANG_SO_PATH_LOADED or _default_language_so_path()
        key = (path, lang)
        if key not in _EXTRA_TS_PARSERS:
            language = Language(path, lang)
            p = Parser()
            p.set_language(language)
            _EXTRA_TS_PARSERS[key] = p
        else:
            p = _EXTRA_TS_PARSERS[key]
    payload = code.encode("utf-8") if isinstance(code, str) else code
    return p.parse(payload).root_node
