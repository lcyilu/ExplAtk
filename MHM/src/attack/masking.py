...
def importance_sampling(src, true_label, wrapper: ModelWrapper, lang='cpp'):
    ...
    # Store results: {identifier: importance_score}
    identifier_scores = {}
    ...
    if np.sum(scores) < 1e-9:
        # If all variables are unimportant, assign equal probabilities to them (uniform distribution)
    ...
    # Sort by probability in descending order for easier inspection, although sampling is random
