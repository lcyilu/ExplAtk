...
def generate_code_db(path_list_file, batch_size=32, device='cuda'):
    ...
    2. Use CodeBERT to compute CLS vectors in batches.
    ...
    try:
        ...
        print("Model loading failed. Please check the network connection or path.")
        raise e
    
    model.eval()

    all_vectors = []    # Temporarily store the vector list.
    valid_paths = []    # Temporarily store successfully processed paths, keeping one-to-one alignment with vectors.
    ...
    for i in tqdm(range(0, total_files, batch_size)):
        ...
        for p in batch_path_chunk:
            ...
            try:
                ...
                # Skip failed reads to keep the data clean.
                pass
        ...
        try:
            ...
            with torch.no_grad():
                ...
                # The shape of last_hidden_state is [batch, seq_len, hidden].
                # [:, 0, :] selects the 0-th token of each sample, i.e. <s>/CLS.
                cls_vecs = outputs.last_hidden_state[:, 0, :]
                
                # Move back to CPU to avoid GPU memory overflow.
                all_vectors.append(cls_vecs.cpu())
                
                # Add paths to the final list only after successful inference.
    ...
    # After normalization, the dot product of two vectors is equal to cosine similarity.
